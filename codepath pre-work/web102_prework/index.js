...
function addGamesToPage(games) {
    ...
    for(let i=0;i<games.length;i++){
        ...
        const info = `
            ...
            <h6>${games[i].name} has raised ${games[i].pledged} from ${games[i].backers}, that had a goal of ${games[i].goal}</h6>
...
function filterUnfundedOnly() {
    ...
    let listGame=GAMES_JSON.filter((game)=> 
        {return game.pledged < game.goal;});

    // use the function we previously created to add the unfunded games to the DOM
    addGamesToPage(listGame);
}
...
let displayStr = `A total of $${totalRaised.toLocaleString()} has been raised for ${totalGames} games. Currently, ${num_of_unfunded} game remains unfunded. We need your help to fund these amazing games!`;
